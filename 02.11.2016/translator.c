...
extern "C" void* NumberBlocks(void* listObject, char* currentNumberOfBlock) {
	
	deque<Block>* listTmp = (deque<Block>*)listObject;
	...
	*tmpStr += currentNumberOfBlock + ( " " + *listTmp->front().translatedBlock );
	delete listTmp->front().translatedBlock;
	
	listTmp->front().translatedBlock = tmpStr;
	
	return listObject;
	// correct mistake -> core dump
}

extern "C" void* AddLabelToListBlock(void* listObject, char* label) {
	
	deque<Block>* listTmp = (deque<Block>*)listObject;
	listTmp->front().numberOfBlock = CounterOfBlocks;
...
extern "C" void* CreateListForString(void* blockStr) {
	...
	switch (address) {
		...
		case M:
			*resultWord = "M" + string(sign) + string(number);
			break;
		case T:
			ValidateExpressionAboutDot(string(number));
